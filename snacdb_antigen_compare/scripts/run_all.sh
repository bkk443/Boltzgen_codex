...
echo "[plan] Step 1/6: Inspecting workbook and generating normalized manifests..."
python "$OUT_DIR/scripts/inspect_excel.py" "$ROOT_DIR/INPUT_ANTIGENS.xlsx" "$OUT_DIR"
...
if compgen -G "$OUT_DIR/02_queries/structures/*" > /dev/null && compgen -G "$OUT_DIR/reference/snacdb_curated/*" > /dev/null; then
  bash "$OUT_DIR/scripts/run_antigen_hits.sh" "$OUT_DIR/reference/snacdb_curated" "$OUT_DIR/02_queries/structures"
else
...
python "$OUT_DIR/scripts/write_summary_report.py"
