...
echo "[plan] Step 5/6: Running SNAC-DB searches if inputs are available..."
if compgen -G "$OUT_DIR/02_queries/structures/*" > /dev/null && compgen -G "$OUT_DIR/reference/snacdb_curated/all_complexes/*" > /dev/null; then
  ...
  echo "Skipping SNAC-DB searches because the query structure directory or reference dataset directory is empty."
fi

echo "[plan] Step 6/6: Postprocessing outputs, building the protein-space map, and writing the summary report..."
python "$OUT_DIR/scripts/postprocess_snacdb_hits.py" "$OUT_DIR"
python "$OUT_DIR/scripts/build_protein_space_map.py" "$OUT_DIR"
